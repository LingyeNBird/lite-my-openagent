...
function getTimestampFileName(date) {
...
async function removeStaleLogs(logDirectory, now) {
...
export async function createRuntimeLogger() {
    ...
    try {
        ...
        await writeFile(filePath, `[${now.toISOString()}] runtime.init ${JSON.stringify({ logDirectory, removedStaleLogs: removedCount })}\n`, "utf8");
        return {
            filePath,
        ...
        return {
            ...
            async log(event, details) {
                console.error(`[lite-my-openagent] skipped log event ${event}: ${JSON.stringify({ details, reason: "logger init failed" })}`);
            },
        };
    }
}
