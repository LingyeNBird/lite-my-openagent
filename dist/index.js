...
function summarizeCommands(config) {
    ...
    const commandRecord = commandValue;
    ...
    return {
        ...
        liteAgentCommandShape: liteAgentCommand && typeof liteAgentCommand === "object" && !Array.isArray(liteAgentCommand)
            ? Object.keys(liteAgentCommand).sort()
    ...
    return {
        ...
        "chat.params": async (input, output) => {
            const messageParts = Array.isArray(input.message.parts)
                ? input.message.parts.filter((part) => typeof part?.type === "string")
                : [];
